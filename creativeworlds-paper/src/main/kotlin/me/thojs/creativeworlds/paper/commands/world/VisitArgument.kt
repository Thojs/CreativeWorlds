    ...
    init {
     ...
     companion object {
         fun visitWorld(visitor: Player, player: OfflinePlayer, id: Int) {
             ...
             if (worlds.isEmpty()) {
                 visitor.sendMessage(me.thojs.creativeworlds.paper.CreativeWorlds.prefix.append(Component.text("No worlds found from ${player.name}.").color(NamedTextColor.RED)))
                 return
             }

             if (id < 0 || worlds.size < id) {
                 visitor.sendMessage(me.thojs.creativeworlds.paper.CreativeWorlds.prefix.append(Component.text("Invalid plot id provided. (${worlds.indices.first+1}, ${worlds.indices.last+1})").color(NamedTextColor.RED)))
                 return
             }
             ...
             if (world.denied.contains(visitor) && (!player.isOp || world.owner != visitor)) {
                 visitor.sendMessage(me.thojs.creativeworlds.paper.CreativeWorlds.prefix.append(Component.text("You are denied from this plot.").color(NamedTextColor.RED)))
                 return
             }

             if (world.bukkitWorld == null) {
                 visitor.sendMessage(me.thojs.creativeworlds.paper.CreativeWorlds.prefix.append(Component.text("Loading world, we will teleport you when the world has been loaded.").color(NamedTextColor.GREEN)))
             } else {
                 visitor.sendMessage(me.thojs.creativeworlds.paper.CreativeWorlds.prefix.append(Component.text("Teleporting you to the world.").color(NamedTextColor.GREEN)))
             }
